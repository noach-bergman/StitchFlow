...
const ClientFolders: React.FC<ClientFoldersProps> = ({ 
  ...
  const generateNewDisplayId = () => (Math.floor(1000 + Math.random() * 9000)).toString();
      ...
      */}
      {activeQrOrder && createPortal(
        <div className="print-only-container">
           ...
           <div style={{ flex: 1, paddingRight: '2mm', display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: 'right', overflow: 'hidden' }}>
              ...
              <p style={{ fontSize: '7pt', margin: '0', color: 'black', opacity: 0.8, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{activeQrOrder.itemType}</p>
           </div>
        </div>,
        document.body
      )}
           ...
           <div className="bg-white rounded-[2.5rem] w-full max-w-lg shadow-2xl animate-in slide-in-from-bottom duration-300 overflow-hidden flex flex-col max-h-[95vh]">
              ...
              {!isGeneratingReceipt && receiptData && (
                 <div className="p-6 border-t border-gray-100 bg-white flex gap-4 shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-10">
                    ...
                    <button onClick={handlePrintReceipt} className="flex-[2] bg-slate-900 text-white font-black rounded-2xl shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2 hover:bg-black">
                       <Printer size={18} /> Print / Share
                    </button>
                 </div>
              )}
           </div>
        ...
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md no-print animate-in fade-in duration-300">
          <div className="bg-white rounded-[3rem] w-full max-w-sm shadow-2xl animate-in zoom-in duration-300 overflow-hidden text-center">
            ...
            <div className="p-10 space-y-6">
              ...
              >
                {isGeneratingQr ? <RefreshCw className="animate-spin" size={20} /> : <Printer size={20} />} 
                {isGeneratingQr ? 'מכין תווית...' : 'הדפסת תווית'}
              </button>
            </div>
          </div>
        </div>
